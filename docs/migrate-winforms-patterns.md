...
description: Map common Windows Forms controls, data-binding patterns, and lifecycle concepts to WinUI 3.
...
| `BackgroundWorker` | `Task` and `async`/`await` | Dispatch only UI work that actually completes on a background thread. |
